-- Migracja 002: Stripe subscriptions
-- Uruchom w Supabase SQL Editor

-- Mapowanie użytkowników Supabase ↔ klientów Stripe
...
-- Instruktor widzi tylko własne subskrypcje
create policy "users_own_subscriptions_select" on subscriptions
  for select to authenticated using (auth.uid() = user_id);

-- Service role może modyfikować (webhook)
